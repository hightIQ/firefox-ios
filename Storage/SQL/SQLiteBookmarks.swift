...
public class SQLiteBookmarks: BookmarksModelFactory {
    ...
    func folderForGUID(guid: GUID, title: String) -> SQLiteBookmarkFolder? {
...
public class SQLiteBookmarkMirrorStorage: BookmarkMirrorStorage {
    ...
    public func applyRecords(records: [BookmarkMirrorItem]) -> Success {
        ...
        let children = records.flatMap { $0.getChildrenArgs() }
        ...
        self.db.transaction(&err) { (conn, err) -> Bool in
            ...
            if !children.isEmpty {
                let chunks = chunk(children, by: BrowserDB.MaxVariableNumber - (BrowserDB.MaxVariableNumber % 3))
                for chunk in chunks {
                    // Hurgh.
                    var parents: Args = []
